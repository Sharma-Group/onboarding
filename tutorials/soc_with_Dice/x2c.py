...
class X2C(lib.StreamObject):
    ...
    def get_hcore(self, mol=None):
        ...
        t1 = xmol.intor_symmetric('int1e_kin')
        v1 = xmol.intor_symmetric('int1e_nuc')
        s1 = xmol.intor_symmetric('int1e_ovlp')
        #w1 = xmol.intor_symmetric('int1e_pnucp')
        w1 = xmol.intor('cint1e_spnucsp_sph', 4) 

        n = t1.shape[0]
        t = numpy.zeros((2*n, 2*n), dtype=numpy.complex)
        v = numpy.zeros((2*n, 2*n), dtype=numpy.complex)
        s = numpy.zeros((2*n, 2*n), dtype=numpy.complex)
        w = numpy.zeros((2*n, 2*n), dtype=numpy.complex)

        t[::2,::2], t[1::2,1::2] = 1.*t1, 1.*t1
        v[::2,::2], v[1::2,1::2] = 1.*v1, 1.*v1
        s[::2,::2], s[1::2,1::2] = 1.*s1, 1.*s1

        for i in range(n):
            for j in range(n):
                w[2*i  , 2*j  ] = w1[3,i,j] + 1j*w1[2,i,j]
                w[2*i+1, 2*j+1] = w1[3,i,j] - 1j*w1[2,i,j]
                w[2*i  , 2*j+1] = 1j*w1[0,i,j] + w1[1,i,j]
                w[2*i+1, 2*j  ] = 1j*w1[0,i,j] - w1[1,i,j]
...
def _x2c1e_hxrmat(t, v, w, s, c):
    ...
    h1 =(h[:nao,:nao] + h[:nao,nao:].dot(x) + x.T.conj().dot(h[nao:,:nao]) + reduce(numpy.dot, (x.T.conj(), h[nao:,nao:], x)))
...
def _1x2c1e_get_hcore(t, v, w, s, c):
    nao = s.shape[0]
    n2 = nao * 2
    h = numpy.zeros((n2,n2), dtype=v.dtype)
    m = numpy.zeros((n2,n2), dtype=v.dtype)
